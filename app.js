...
function renderCaja() {
  app.innerHTML = `${pageHeader('Nuevo pedido', 'Cargá los detalles desde la notebook y cada sector recibirá solamente lo que le corresponde.')}<div class="layout caja-layout"><form id="order-form" class="card caja-form"><div class="form-section"><h2>Pedido ${nextNumber()}</h2><label class="field-label">Nombre o referencia (opcional)</label><input id="customer" placeholder="Ej: mesa 4" /></div>${simpleForm('pancho', 'Panchos', ['Pancho clásico', 'Pancho especial'])}${simpleForm('hamburguesa', 'Hamburguesas', ['Hamburguesa clásica', 'Hamburguesa completa', 'Hamburguesa veggie'])}${pizzaForm()}<div class="form-section"><label class="field-label">Notas generales</label><textarea id="notes" placeholder="Ej: entregar todo junto"></textarea></div><button class="btn btn-secondary full" type="submit">Enviar pedido a cocina</button></form><section><div class="stats"><div class="stat"><strong>${orders.filter(o => o.status !== 'entregado').length}</strong><span>pedidos activos</span></div><div class="stat"><strong>${orders.filter(o => o.status === 'listo').length}</strong><span>listos</span></div></div>${renderOrderList(orders.filter(o => o.status !== 'entregado'), 'Pedidos recientes')}</section></div>`;
  bindEditActions();
  const draft = { panchos: [], hamburguesas: [], pizzas: [] };
  const ingredientOptions = { pancho: ['Mayonesa', 'Ketchup', 'Mostaza', 'Papas pay'], hamburguesa: ['Tomate', 'Lechuga', 'Mayonesa', 'Ketchup'] };
  Object.entries(ingredientOptions).forEach(([type, options]) => { const section = document.querySelector(`.${type}-section .detail-control`); section.insertAdjacentHTML('beforebegin', `<div class="ingredient-control"><label class="field-label">Sabores</label><div class="choice-grid ingredient-grid">${options.map(option => `<div class="choice"><input id="${type}-${option.toLowerCase().replaceAll(' ', '-')}-flavor" type="checkbox" value="${option}" /><label for="${type}-${option.toLowerCase().replaceAll(' ', '-')}-flavor">${option}</label></div>`).join('')}</div></div>`); });
  document.querySelector('#hamburguesa-product').addEventListener('change', event => { const complete = event.target.value.toLowerCase().includes('completa'); document.querySelectorAll('.hamburguesa-section .ingredient-control input').forEach(input => { input.checked = complete; }); });
  const pizzaQty = document.querySelector('#pizza-qty'); pizzaQty.outerHTML = `<div class="pizza-quantity-control"><label class="field-label" for="pizza-qty">Cantidad</label>${pizzaQty.outerHTML}</div>`;
  ['pancho', 'hamburguesa'].forEach(type => document.querySelector(`#add-${type}`).addEventListener('click', e => { e.preventDefault(); e.stopImmediatePropagation(); const key = type === 'pancho' ? 'panchos' : 'hamburguesas'; const ingredients = [...document.querySelectorAll(`.${type}-section .ingredient-control input:checked`)].map(input => input.value); const extra = document.querySelector(`#${type}-detail`).value.trim(); const detail = [...ingredients, extra].filter(Boolean).join(', '); const item = { name: document.querySelector(`#${type}-product`).value, quantity: Number(document.querySelector(`#${type}-qty`).value), detail }; draft[key].push(item); document.querySelector(`#${type}-lines`).innerHTML += `<div class="item-line"><span>${item.quantity} × ${escapeHtml(item.name)}<small class="item-detail">${item.detail ? ` · ${escapeHtml(item.detail)}` : ''}</small></span><span>✓</span></div>`; document.querySelectorAll(`.${type}-section .ingredient-control input`).forEach(input => input.checked = false); }, true));
  document.querySelectorAll('.pizza-section input[type="checkbox"]').forEach(input => input.addEventListener('change', () => { const selected = [...document.querySelectorAll('.pizza-section input[type="checkbox"]:checked')].map(checkbox => checkbox.value); const allowedMix = selected.length === 2 && selected.every(flavor => ['Panceta', 'Huevo'].includes(flavor)); if (selected.length > 1 && !allowedMix) { input.checked = false; showToast('Solo se puede combinar Panceta + Huevo.'); } }));
  document.querySelector('#add-pizza').addEventListener('click', e => { e.preventDefault(); e.stopImmediatePropagation(); const flavors = [...document.querySelectorAll('.pizza-section input[type="checkbox"]:checked')].map(input => input.value); if (!flavors.length) return showToast('Elegí al menos un sabor de pizza.'); if (flavors.length > 1 && !(flavors.length === 2 && flavors.every(flavor => ['Panceta', 'Huevo'].includes(flavor)))) return showToast('Solo se puede combinar Panceta + Huevo.'); const item = { name: `Pizza ${flavors.join(' y ')}`, quantity: Number(document.querySelector('#pizza-qty').value), detail: flavors.includes('Caprese') ? 'Albahaca y tomate' : '', status: 'pendiente' }; draft.pizzas.push(item); document.querySelector('#pizza-lines').innerHTML += `<div class="item-line"><span>${item.quantity} × ${escapeHtml(item.name)}<small class="item-detail">${item.detail}</small></span><span>✓</span></div>`; document.querySelectorAll('.pizza-section input[type="checkbox"]').forEach(input => input.checked = false); }, true);
